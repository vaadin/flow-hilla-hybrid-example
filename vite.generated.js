...
function statsExtracterPlugin() {
    ...
    return {
        ...
        async writeBundle(options, bundle) {
            ...
            const cvdls = Object.fromEntries(npmModules
            ...
            const customIndexData = readFileSync(projectIndexHtml, { encoding: 'utf-8' });
            const generatedIndexData = readFileSync(generatedIndexHtml, {
                encoding: 'utf-8'
            });
            ...
            const isThemeComponentsResource = (id) => id.startsWith(themeOptions.frontendGeneratedFolder.replace(/\\/g, '/'))
                && id.match(/.*\/jar-resources\/themes\/[^\/]+\/components\//);
            const isGeneratedWebComponentResource = (id) => id.startsWith(themeOptions.frontendGeneratedFolder.replace(/\\/g, '/'))
                && id.match(/.*\/flow\/web-components\//);
            const isFrontendResourceCollected = (id) => !id.startsWith(themeOptions.frontendGeneratedFolder.replace(/\\/g, '/'))
            ...
            generatedImports
                .filter((line) => line.includes('generated/jar-resources'))
                .forEach((line) => {
            ...
            generatedImports
                .filter((line) => line.startsWith(frontendFolderAlias + '/'))
                .filter((line) => !line.startsWith(frontendFolderAlias + '/generated/'))
                .filter((line) => !line.startsWith(frontendFolderAlias + '/themes/'))
                .map((line) => line.substring(frontendFolderAlias.length + 1))
                .filter((line) => !frontendFiles[line])
                .forEach((line) => {
                const filePath = path.resolve(frontendFolder, line);
...
function themePlugin(opts) {
    ...
    return {
        ...
        async transform(raw, id, options) {
            ...
            if ((!bareId?.startsWith(themeFolder) && !bareId?.startsWith(themeOptions.themeResourceFolder)) ||
                !bareId?.endsWith('.css')) {
                return;
            }
