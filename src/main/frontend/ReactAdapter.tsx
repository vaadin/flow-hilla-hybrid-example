...
export abstract class ReactAdapterElement extends HTMLElement {
    ...
    #contents = new Map<string, Element>();
    ...
    public async connectedCallback() {
        for (const child of this.children) {
            ...
            if (child.localName === 'flow-content-container' && (name = child.getAttribute('name'))) {
                this.#contents.set(name, child);
            }
        }
    ...
    protected useContent(name: string): ReactElement | null {
        ...
        useEffect(() => {
            if (this.#contents.has(name)) {
                ref.current?.appendChild(this.#contents.get(name)!);
            }
        }, []);
        return createElement('flow-content-placeholder', {name, ref, style: {display: 'contents'}});
    }
